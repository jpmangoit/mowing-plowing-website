...
<form class="needs-validation mt-3" id="add-card-form" action="{{route('admin.recurring-jobs.scheduledate')}}" method="post">
    @csrf
    <div class="card">
        ...
        <div class="card-footer px-0 border-0 text-center">
            ...
            <input type="hidden" name="provider_id" value="{{$job->provider_id}}">
...
<script>
    ...
    $('#add-card-form').on('submit', function(e) {
        ...
        $.ajax({
            ...
            url: "{{ route('admin.recurring-jobs.scheduledate') }}",
            type: 'post',
            data: $(this).serialize(),
